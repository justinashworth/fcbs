...
static t_float const MAGIC_BIGNUM = 456e13;
...
void
pearson_distances(Matrix const & matrix, Indices const & colinds, t_float * const dist, bool pairwise_complete_obs=true){
...
void
spearman_distances(Matrix const & matrix, Indices const & colinds, t_float * const dist){
    
    // precompute ranks for all rows
    size_t const nrow(matrix.size()), ncol(colinds.size());
    IndMatrix rankorders(nrow);
    Matrix ranks(nrow);
    for(size_t row(0); row<nrow; ++row){
        ...
        Indices & rankorder(rankorders[row]);
...
//        std::cout << std::endl;
        ...
        // compute ranks
        //ranks[row].assign(ncol,MAGIC_BIGNUM);
        Ties ties;
        double last(0);
        unsigned vali(0);
        for(size_t i(0); i<ncol; ++i){
            ...
            if(val==MAGIC_BIGNUM){
                ranks[row].push_back(val);
                continue;
            }
            // ranks are 1-indexed (though this shouldn't matter?)
            else ranks[row].push_back(vali+1);
            ...
            if(val == last){
                ...
                rank /= ties.size();
                for(size_t ip(0); ip<ties.size(); ++ip) ranks[row][i-ip-1] = rank;
                ties.clear();
            }
        ...
        if(!ties.empty()){
            ...
            rank /= ties.size();
            for(size_t ip(0); ip<ties.size(); ++ip) ranks[row][vali-ip-1] = rank;
        }
    }
    
    // pairwise pearson correlations of the pre-computed spearman ranks
    Values xo(ncol,0), yo(ncol,0);
    std::ptrdiff_t p(0);
    t_float EX(0.0), EY(0.0), EXX(0.0), EYY(0.0), EXY(0.0), x(0.0), y(0.0), d(0.0), num(0.0), denom(0.0);
    size_t r1(0), r2(0), i(0);
    unsigned npairs(0);
    for(r1=0; r1<(nrow-1); ++r1){
		for(r2=r1+1; r2<nrow; ++r2){
            ...
            // this 'unsort' is currently the slowest step in profiling (60% of time spent here!)
            for(i=0; i<ncol; ++i){
                xo[rankorders[r1][i]] = ranks[r1][i];
                yo[rankorders[r2][i]] = ranks[r2][i];
            }
            ...
            for(i=0; i<ncol; ++i){
                ...
                if(x==MAGIC_BIGNUM || y==MAGIC_BIGNUM) continue;
                ++npairs;
                EX += x;
                EY += y;
                EXX += x*x;
...
void
read_merge_file(std::string const & filename, Merge & merge)
{
	// open input file
	std::ifstream file;
	file.open( filename.c_str() );
...
void
get_members(Merge const & merge, Members & members){
...
void
output_node_counts(Counts const & nodecounts, std::string prefix="")
{
    std::ofstream of;
